...
def Nearest_Neighbor_Heuristic(graph_settings):
    ...
    while(a < tamV):
        ...
        while(capacity == False):
            ...
            j = 0
            while(j < tamN):
                if(int(Nodes[vInsertAux-1].idNode) != int(Nodes[j].idNode) and Nodes[j].visit == False):
            ...
            if(int(Vehicles[a].capacity) < int(Nodes[vInsert-1].demand) or int(somDemand == graph_settings["DEMAND_TOTAL"])):
                capacity = True
        Vehicles[a].roteNodes.append(int(1))
        a += 1
    for a in Vehicles:
